                ...
                <div class="inner-box">
                    ...
                    <div class="forms-wrap">
                        ...
                        <div class="form sign-in-form">
                            ...
                            <div class="heading">
                                ...
                                <a href="#/user/login" class="toggle" style="color: red;">Sign up</a>
                            </div>
                            <!-- 包含输入字段和提交按钮的容器 -->
                            <div class="actual-form">
                                ...
                                <div class="input-wrap">
                                    <input id="name" type="text" 
                                        class="input-field" v-model="loginParams.userAccount">
                        ...
                        <div  class="form sign-up-form">
                            ...
                            <div class="heading">
                                ...
                                <a href="#/user/login" class="toggle" style="color: red;">Sign In</a>
                            </div>
                            <!-- 包含输入字段和提交按钮的容器 -->
                            <div class="actual-form">
                                <!-- 包含用户名输入字段的容器 -->
                                <div class="input-wrap">
                                    <input id="sign-name" type="text" 
                                        class="input-field" v-model="registerParams.userAccount">
                                    <label for="sign-name">Name</label>
                                </div>
                                <!-- 包含电子邮件输入字段的容器 -->
                                <div class="input-wrap">
                                    <input type="password" v-model="registerParams.userPassword" id="sign-email"  class="input-field">
                                    <label for="sign-email">Password</label>
                                </div>
                                <!-- 包含密码输入字段的容器 -->
                                <div class="input-wrap">
                                    <input id="sign-password" type="password" autocomplete="off" required
                                        class="input-field" v-model="registerParams.checkPassword">
                                    <label for="sign-password">CheckPassword</label>
                                </div>
                                <!-- 注册表单的提交按钮 -->
                                <button class="sign-btn" @click="handleRegister">SIGN UP</button>
                                <!-- 同意服务条款和隐私协议的复选框 -->
                    ...
                    <div class="carousel">
                        ...
                        <div class="images-wrapper">
                            <img src="../../assets/images/picture1.jpeg" alt="" class="image img-1 show">
                            <img src="../../assets/images/picture2.png" alt="" class="image img-2">
                            <img src="../../assets/images/picture3.jpeg" alt="" class="image img-3">
                        </div>
                        <!-- 包含轮播图文本和轮播图指示器的容器 -->
...
const handleLogin=async()=>{
    const res = await UserControllerService.userLoginUsingPost(loginParams)
    if(res.code === 0){
        //登录成功
        userStoreInfo.updateUser(res?.data)
        ...
        Message.error("登陆失败")
        loginParams.userAccount=''
        loginParams.userPassword=''
    }
}
...
const handleRegister = async()=>{
    ...
    if(res.code===0){
        Message.success({
            content:"注册成功,请至登录页进行登录",
            duration:3000
        })
        registerParams.userAccount = ""
        registerParams.checkPassword = ""
...
onMounted(() => {
    ...
    // 
    inputs.value = document.querySelectorAll('.input-field')
    toggleBtns.value = document.querySelectorAll('.toggle')
    main.value = document.querySelector('main')
    bullets.value = document.querySelectorAll('.bullets span')
    images.value = document.querySelectorAll('.image')
    ...
    toggleBtns.value.forEach(btn => {
        btn.addEventListener('click', () => {
            main.value.classList.toggle('sign-up-mode')
        })
    })
...
const moveSlider = (index) => {
    ...
    images.value.forEach((img, i) => {
        img.classList.toggle('show', i === currentIndex.value)
    })
    ...
    bullets.value.forEach((bullet, i) => {
        bullet.classList.toggle('active', i === currentIndex.value)
    })
}
...
.login-page {
 ...
 main {
    ...
    .box {
        ...
        background-color: rgba(228, 228, 228, 0.133);
        opacity: 0.9;
        position: relative;
        /* 设置.box的定位为相对定位 */
